...
Use one app registration for the IBM i MCP resource API and a separate app registration for the Claude OAuth client. This keeps the API's audience and permissions distinct from the credentials Claude uses during authorization.
...
<Step title="Issue v2 access tokens">
  Open the app registration's **Manifest** and set
  `api.requestedAccessTokenVersion` to `2`. The server uses Entra's `/v2.0`
  issuer and expects a v2 access token.
</Step>

<Step title="Expose the delegated scope">
  Under **Expose an API**, set the Application ID URI to the canonical MCP
  resource URL, such as `https://mcp.example.com/mcp`. Add a delegated scope
  named `ibmi.read` that administrators and users can consent to as appropriate
  for your tenant.
</Step>
...
# Must match the URL users enter in their MCP client exactly, path included
...
| `OAUTH_SCOPES_SUPPORTED` | Comma-separated, fully-qualified scopes advertised to clients and included in bearer challenges.                                                          |
...
<Warning>
Entra requires the **fully-qualified** scope URI in the authorization request, so `OAUTH_SCOPES_SUPPORTED` uses `<application-id-uri>/<scope>`. The issued token's own `scp` claim contains only the short name (`ibmi.read`), which is why `OAUTH_REQUIRED_SCOPES` uses the short value.
</Warning>
...
```http
WWW-Authenticate: Bearer resource_metadata="https://mcp.example.com/.well-known/oauth-protected-resource/mcp", scope="https://mcp.example.com/mcp/ibmi.read"
```
...
| `AADSTS9010010` on token request                                         | The MCP server URL does not match the resource API's Application ID URI.                                                                                                      |
