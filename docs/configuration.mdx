...
| `MCP_LOG_TO_FILE` | Write logs to rotating files in `LOGS_DIR` | `true` | `false` to log to the console only |
...
<Note>
Set `MCP_LOG_TO_FILE=false` to skip file logging entirely. Logs then go only to
stderr as JSON, which is what you want when the platform already collects
stderr (ECS `awslogs`, Docker, Kubernetes, systemd) or when the container has no
writable log directory. `LOGS_DIR` is ignored in that case, and nothing is
created on disk.
</Note>
